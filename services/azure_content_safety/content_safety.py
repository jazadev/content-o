...
def validate_with_content_safety(content, endpoint, subscription_key, api_version, severity_threshold="high"):
...
def validate_content(content, llm, severity_threshold="high"):
    """
    Valida el contenido (input o output) usando Content Safety para asegurarse
    de que no contenga información confidencial o sensible.

    Args:
        content (str): El contenido a validar.
        llm (AzureOpenAI): Cliente Azure OpenAI configurado para Content Safety.
        severity_threshold (str): Nivel de severidad permitido ("low", "medium", "high").

    Returns:
        bool: True si el contenido es seguro, False si se detecta algo no permitido.
    """
    try:
        response = llm.run(content)
        flagged = response.get("flagged", False)
        severity = response.get("severity", "")
        if flagged and severity in ["medium", "high"]:
            logging.warning(f"Contenido no permitido detectado: {response}")
            return False
        return True
    except Exception as e:
        logging.error(f"Error al validar contenido: {e}")
        return False
